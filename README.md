...
李宇轩体系另包含 **6,240 张结构化视觉审阅帧**和 **7,943 张密集时序 Pose 帧**。这些材料只聚合为公开安全的来源级证据；仓库不包含原视频、音频、完整转写、截图、Pose 坐标或模型原始输出。唯一例外是经项目所有者明确批准、带原平台归属的 [刘辉七项 Pages 案例](docs/github-pages-demo.md#七个公开媒体例外)：每项包含 7 张阶段帧和 1 段连续短片。
...
| **发布隔离** | Git 不包含上传媒体、参考帧缓存、数据库、日志、访问令牌或模型原始输出；唯一例外是经明确批准并带来源归属的刘辉 Pages 七项案例 |
