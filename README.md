...
| [**郑思维混双 Skill**](skills/zheng-siwei-badminton-coach/SKILL.md) | 两人是否保有两条可用通道、下一拍是否有明确归属？ | 混双接发、后场进攻连续性、前场压迫、轮转、防守转换与回位 | 21 条公开来源索引 · 2 个已审视觉时间点 |
...
李宇轩体系另包含 **6,240 张结构化视觉审阅帧**和 **7,943 张密集时序 Pose 帧**。这些材料只聚合为公开安全的来源级证据；仓库不包含原视频、音频、完整转写、截图、Pose 坐标或模型原始输出。唯一例外是经项目所有者明确批准、带原平台归属的 [刘辉高远球 Pages case](docs/github-pages-demo.md#单一公开媒体例外)：7 张阶段帧和 1 段连续短片。
...
| **发布隔离** | Git 不包含上传媒体、参考帧缓存、数据库、日志、访问令牌或模型原始输出；唯一例外是经明确批准并带来源归属的刘辉 Pages 单一 case |
