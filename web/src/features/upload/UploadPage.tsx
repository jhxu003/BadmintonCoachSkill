...
export function UploadPage({ onCreated }: UploadPageProps) {
  ...
  async function submit(): Promise<void> {
    ...
    form.set("player_profile", JSON.stringify({ training_goal: "technique_diagnosis" }));
    try {
  ...
  return (
    <main className="upload-page">
      <section className="upload-intro">
        ...
        <h1>让每一次纠正回到动作画面。</h1>
        <p className="upload-lede">上传学员视频后，系统会定位动作阶段、匹配教练体系，并把诊断问题绑定到学员与教练的对应关键帧。</p>
      ...
      <section className="upload-surface" aria-label="创建视频分析">
        ...
        <label>教练体系<select value={coach} onChange={(event) => setCoach(event.target.value)}><option value="liu-hui">刘辉</option><option value="li-yuxuan">李宇轩</option></select></label>
        <label>主要动作<select value={action} onChange={(event) => setAction(event.target.value)}><option value="high_clear">后场高远球</option><option value="smash">杀球</option><option value="drop">吊球</option><option value="rear_footwork">后场步法</option><option value="drive">平抽挡</option></select></label>
