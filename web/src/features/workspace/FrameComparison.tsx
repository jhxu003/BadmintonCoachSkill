...
interface FrameComparisonProps {
  ...
  coachFrames: Map<string, FrameRef>;
}
    ...
    <section className="comparison-panel">
      ...
      <div className="comparison-grid">
        <article className="evidence-frame">
          ...
          {student ? <img src={student.media_url ?? studentFrameUrl(analysisId, student.frame_id)} alt="学员动作关键帧" /> : <EmptyFrame text="当前视频没有足够画面支持该阶段判断" />}
        ...
        <article className="evidence-frame">
          ...
          {coach?.media_url ? <img src={coach.media_url} alt="教练动作参考关键帧" /> : <EmptyFrame text={unavailable ? "证据不足，请补拍对应机位" : "该公开来源的画面缓存暂不可用"} />}
...
function FrameFacts({ frame }: { frame?: FrameRef }) {
