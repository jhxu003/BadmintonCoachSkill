...
- Git must not contain uploads, frames, model outputs, private caches, database files, logs, or access tokens. The only exceptions are the seven owner-approved, source-attributed Liu Hui Pages demonstration asset sets documented in [`github-pages-demo.md`](github-pages-demo.md); they contain no learner media or runtime cache.
